...
- 矩阵：关卡选择表、雪量
- 图：界面跳转、怪物寻路（最短路径）
- 线性表：不规则的障碍物碰撞体积的顶点、关卡选择系列
- 散列表：单位状态、事件侦听器、按键映射表
...
###Obstacles
障碍物为树、楼房（教学楼、店铺）、垃圾桶、汽车、石头、雪人。碰撞体积为圆形（后可以改成不规则多边形）。在游戏过程中会自动出现在地图的随机地方。现设定障碍物覆盖面积约为 30%。有以下属性：
